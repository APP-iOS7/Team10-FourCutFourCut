...
### 조영민
- **SwiftUI 이미지 데이터 방식**: 이미지 저장 방법과 이미지 데이터 처리 흐름에 대해서 공부할 수 있었습니다.
- **뷰 분리와 의존성 관리**: 뷰를 적절히 분리하는 것이 의존성을 낮추고 코드의 재사용성과 유지보수성을 높이는 데 중요하다는 것을 깨달았습니다.
- **개발자는 소통이 중요하다**: 서로 생각이 달라서 의사소통을 할 때 서로가 잘못 이해하는 순간들이 종종 있었습니다. 하지만 이러한 부분을 통해서 오히려 소통의 중요성을 느낄 수 있었습니다.
